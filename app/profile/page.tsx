              ...
              <div className="mb-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {userInfo.skills.map((skill) => (
                    <span key={skill} className="inline-block bg-gray-100 text-gray-800 px-3 py-1 rounded-md text-sm">
                      {skill}
                    </span>
                  ))}
                </div>
                ...
                {proposedProjects.length > 0 ? (
                  proposedProjects.map((project) => (
                    <div key={project.id} className="bg-white border border-gray-200 rounded-lg p-4">
                      <div className="flex justify-between items-start mb-2">
                        ...
                        <span
                          className={`text-xs px-2 py-1 rounded-full ${
                            project.status === '모집중' ? 'bg-green-50 text-green-700' : 'bg-blue-50 text-blue-700'
                          }`}
                        >
                          {project.status}
                        </span>
                      ...
                      <p className="text-xs text-gray-500">지원자 {project.applicants}명</p>
                    </div>
                  ))
                ) : (
                  <div className="bg-white border border-gray-200 rounded-lg p-8 text-center">
                    <p className="text-gray-500 text-sm">제안한 프로젝트가 없습니다</p>
                  </div>
                )}
              </div>
            </div>
              ...
              <div>
                ...
                <div className="space-y-4">
                  {acceptedProjects.length > 0 ? (
                    ...
                    <div className="bg-white border border-gray-200 rounded-lg p-8 text-center">
                      <p className="text-gray-500 text-sm">참여중인 프로젝트가 없습니다</p>
                    </div>
                  )}
                </div>
              </div>
